...
#Global Centrality
#Eigen
#e_values <- projected %>% as_adjacency_matrix(type="both") %>% eigen()
...
#mean(close_cent) #0.0000001142723
...
#So the distribution of Bonacich power is slightly positively skewed meaning that in general, vertices are considered more 'powerful' as their alters increase in power.
```
...
#max(page_ranks$vector) # 0.01541433
...
# Eigen values are negatively skewed for global community 2. This means that most nodes are not connected to high scoring nodes.
```
