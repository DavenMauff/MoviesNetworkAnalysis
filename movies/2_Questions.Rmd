...
```{r}
movie_metadata <- read.csv("../data-raw/movies_metadata.csv", sep=";")
```
...
revenue_data <- movie_metadata %>%
  filter(movies_metadata$gross >= 10000000 & movies_metadata$gross >= 10000000) %>%
...
ggplot(movie_metadata, aes(imdb_score)) +
 ...
 ggtitle("Histogram for IMDB ratings") +
  annotate("text", label = "Mean IMDB rating",x = 6.2, y = 50, size = 100, colour = "yellow",angle=90)
```
