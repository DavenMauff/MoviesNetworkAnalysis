...
```{r setup, include=FALSE}
library(pacman)
pacman::p_load(dplyr, ggplot2, tidyr, ggrepel, formattable, ggcorrplot, GGally, rpart.plot, rpart, VIM, caret, stringr, ggthemes, corrplot, magrittr, plotly)
knitr::opts_chunk$set(echo = TRUE)
setwd(getwd())
```
...
```{r}
revenue_data<- movie_metadata%>%
  filter(movie_metadata$budget >= 1000000 & movie_metadata$revenue >= 1000000) %>%
