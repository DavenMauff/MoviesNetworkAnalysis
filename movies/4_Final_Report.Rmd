...
In the next section we can introduce our dataset and look at its content. 
...
We remove `aspect_ratio` because 1 it has a lot of missing values and 2 we will not be looking at the impact that it has on other data (we assume that it doesn't). 
...
As you can see, we have gross and budget information. So let’s add two colums: profit and percentage return on investment for further analysis.
...
Next, we take a look at the `colour` vs `black and white`. 
...
We can also look at the different types of countries.
...
ggplot(movie_metadata, aes(title_year)) +
  ...
  ggplot2::annotate("text", label = "Year = 1980",x = 1979, y = 60, size = 3, colour = "blue",angle=90)
```
...
Let's have a look at the movie counts again. This looks much better to work with, as there are now a decent number of movies for every year.
...
Here we split the genres and look at how they often they occurred since 1980. We split the genres separated by "|" into 8 different columns.
